...
// the array position_party_2 gives the positions of persons from party_2 in the
// unified outcome party. all position values in the array are in the range
// 1 .. length of outcome party. can assume that the given array values are legal
// and ensure that the relative order between the persons from party_1 is kept the same
// in the outcome party, and so also the relative order between the persons from party_2 is kept
// the same in the outcome party. if the array is shorter than the size of party_2,
// then the relevant persons from party_2 that do not have positions in the array
// are positioned in the last positions in the outcome party (keep relative order between them). 
// if operation succeeds, then the outcome party is created and given by the output parameter
// outcome_party. in this case the 2 original parties are destroyed and become NULL (therefore they
// are also output parameters).
// if fails, then the 2 original parties stay.
...
// name and code can be here of any length. (the limit of 50 chars is NOT relevant here).
// can assume that both name and code do not include space, nor tab, nor new line.
// size is any int number (positive, at least 1). 
